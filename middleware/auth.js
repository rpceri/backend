...
module.exports = (req, res, next) => {
  try {
    ...
    const decodedToken = jwt.verify(token, 'RANDOM_TOKEN_SECRET'); /// on decode le token qui devient un object javascript classique, on va pouvoir recup le userId qu'il y a dedans
    ...
    if (req.body.userId && req.body.userId !== userId) { // si il y a un user id dans le corp de la requete et que celui ci est différent de celui ci...
      ...
      req.auth = { userId };   // dans un middleware on peut rajouter des propriétés qui seront accessibles au suivant.. on fait cela pour sécuriser le delete
      //{ userId } est un racccourci javascript de : { userId : userId }, possible quand meme nom, ainsi dans deletething on aura req.auth.userId
      next();
    }
  } catch (error) {
    res.status(401).json({
